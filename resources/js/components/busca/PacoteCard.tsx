...
export default function PacoteCard({ pacote }: { pacote: Pacote }) {
    const destino = pacote.ofertas[0]?.hotel?.cidade?.nome || 'Destino Desconhecido';
    const fotoUrl = pacote.fotosDoPacote?.fotoDoPacote || '/placeholder.jpg';
        ...
        <div className="flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all hover:shadow-xl">
            <div className="relative h-48 overflow-hidden">
                ...
                {pacote.ofertas[0]?.status === 'CONCLUIDO' && (
                    <div className="absolute right-2 top-2 rounded bg-gray-800 px-2 py-1 text-xs text-white opacity-80">
                        Encerrado
                    </div>
                )}
            </div>
            <div className="flex flex-1 flex-col p-5">
                ...
                <p className="mb-2 flex items-center text-sm text-gray-500">
                    <MdOutlineMyLocation className="mr-1 text-xl" />
                    {destino}
                </p>
                ...
                <div className="block items-end justify-between border-t border-gray-100 pt-4">
                    <div>
                        <p className="flex items-center text-xs uppercase text-gray-400">
                            <FaMoneyCheckAlt className="mr-1 text-xl" /> A partir de
                        </p>

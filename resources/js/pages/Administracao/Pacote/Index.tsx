...
import {
    create as createPacote,
    edit as editPacote,
} from '@/routes/administracao/pacote';
import {
    deletarPacote,
    obterVendas,
    type CompraRelatorio,
} from '@/services/catalogo/pacoteService';
...
export default function Index({ pacotes = [], success }: Props) {
    const [selectedPacote, setSelectedPacote] = useState<PacoteData | null>(
        null,
    );
    const [salesList, setSalesList] = useState<CompraRelatorio[]>([]);
    ...
    const fetchSales = async (pacote: PacoteData) => {
        ...
        try {
            const data = await obterVendas(pacote.id);
            setSalesList(data);
        } catch (error) {
                        ...
                        <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0 z-10">
                            <div>
                                <h3 className="text-2xl font-black text-gray-900 leading-tight">
                                    Vendas: {selectedPacote.nome}
                                </h3>
                                <p className="text-gray-500 text-sm font-medium">
                                    Relatório de compras efetuadas para este
                                    pacote.
                                </p>
                            </div>
                            <button
                        ...
                        <div className="p-6 overflow-y-auto flex-1 bg-gray-50/50">
                            {loadingSales ? (
                                ...
                                <div className="py-20 text-center flex flex-col items-center gap-4">
                                    ...
                                    />
                                    <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">
                                        Nenhuma venda registrada para este
                                        pacote ainda.
                                    </p>
                                </div>
                            )}

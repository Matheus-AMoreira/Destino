...
export default function Edit({ pacoteFoto }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        _method: 'POST', // Used for spoofing if needed, but we used POST route for update
    });

    const [preview, setPreview] = useState<string | null>(pacoteFoto.foto_do_pacote);
                ...
                <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl bg-white p-8 shadow-sm border border-gray-100">
                    ...
                    <div>
                        <label className={labelClasses}>
                            ...
                            Foto de Capa (Deixe vazio para manter a atual)
                        </label>
